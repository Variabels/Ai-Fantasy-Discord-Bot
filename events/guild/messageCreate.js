...
module.exports = async (client, interaction) => {
  try {
    //if the interaction is not in a guild (aka in dms), return aka ignore the inputs
    if (!interaction.guild) return;
    // if the interaction  author is a bot, return aka ignore the inputs
    if (interaction.author.bot) return;
    //if the channel is on partial fetch it
    if (interaction.channel.partial) await interaction.channel.fetch();
    //if the interaction is on partial fetch it
    ...
    if (!prefixRegex.test(interaction.content)) return;
    //now define the right prefix either ping or not ping
    const [, matchedPrefix] = interaction.content.match(prefixRegex);
    //create the arguments with sliceing of of the rightprefix length
    const args = interaction.content.slice(matchedPrefix.length).trim().split(/ +/);
    ...
    if (command){
        ...
        if (timestamps.has(interaction.author.id)) { //if the user is on cooldown
          const expirationTime = timestamps.get(interaction.author.id) + cooldownAmount; //get the amount of time he needs to wait until he can run the cmd again
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            ); //send an information interaction
          }
        }
        timestamps.set(interaction.author.id, now); //if he is not on cooldown, set it to the cooldown
        setTimeout(() => timestamps.delete(interaction.author.id), cooldownAmount); //set a timeout function with the cooldown, so it gets deleted later on again
      try{
        //if Command has specific permission return error
        if(command.memberpermissions && !interaction.member.hasPermission(command.memberpermissions, { checkAdmin: command.adminPermOverride, checkOwner: command.adminPermOverride })) {
          return interaction.channel.send(new Discord.MessageEmbed()
            .setColor(ee.wrongcolor)
            .setFooter({ text : ee.footertext, iconURL: ee.footericon})
        ...
        command.run(client, interaction, args, interaction.member, args.join(" "), prefix);
      }catch (e) {
