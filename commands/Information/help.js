    ...
    run: async (client, message, args, user, text, prefix) => {
      try{
        if (args[0]) {
          ...
          const extrahelp = `To see command descriptions and inforamtion, type: ${config.prefix}help [CMD NAME]`;
          const embed = new MessageEmbed()
              .setColor(ee.color)
              .setThumbnail(client.user.displayAvatarURL())
              .setTitle("HELP MENU 🔰 Commands")
              .setFooter({ text: extrahelp, iconURL: client.user.displayAvatarURL()});
